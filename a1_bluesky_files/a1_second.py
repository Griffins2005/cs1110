...
def behead(s, marker):
    ...
    """
    pos = s.index(marker)
    length_string = pos + len(marker)
    return s[length_string:] # STUDENTS: replace with your implementation
...
def extract(s, left_marker, right_marker):
    ...
    # STUDENTS: replace with your implementation.
    left = behead(s, left_marker)
    right = left.index(right_marker)
...
def post_info(bp):
    ...
    text_str = behead(bp, 'data-testid="postText"')
    post_text = extract(text_str, '>', '</div>')
    like_num = extract(bp, '<button aria-label="Like (', 'like')

    return like_num + "likes for: " + post_text
    # STUDENTS: replace with your implementation
